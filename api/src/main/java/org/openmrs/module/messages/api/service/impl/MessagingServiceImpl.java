...
public class MessagingServiceImpl extends BaseOpenmrsDataService<ScheduledService> implements MessagingService {
    ...
    public ActorResponse registerResponse(Integer actorId, Integer patientId, String sourceId, String sourceType,
                                          Integer questionId, String textQuestion, Integer responseId, String textResponse,
                                          Date timestamp) {
    ...
    // that's why to require so many parameters
    @SuppressWarnings("checkstyle:ParameterNumber")
    public ScheduledService registerResponseAndStatus(Integer scheduledId, Integer questionId, String textQuestion,
                                                      Integer responseId, String textResponse, String status, Date timestamp,
    ...
    @SuppressWarnings("checkstyle:ParameterNumber")
    public ScheduledService registerResponseAndStatus(Integer scheduledId, Integer questionId, String textQuestion,
                                                      Integer responseId, String textResponse, ServiceStatus status,
                                                      Date timestamp, String executionId) throws PropertyValueException {
        registerResponseForScheduledService(scheduledId, questionId, textQuestion, responseId, textResponse, timestamp);
        return registerAttempt(scheduledId, status, timestamp, executionId);
    }
    ...
    public List<ServiceResultList> retrieveAllServiceExecutions(Integer patientId, ZonedDateTime startDate,
                                                                ZonedDateTime endDate) {
        PatientTemplateCriteria patientTemplateCriteria = PatientTemplateCriteria.forPatientId(patientId);
        return retrieveAllServiceExecutions(patientTemplateService.findAllByCriteria(patientTemplateCriteria), startDate,
                endDate, true);
    }

    @Override
    @Transactional(noRollbackFor = {RuntimeException.class, SQLGrammarException.class}, readOnly = true)
    public List<ServiceResultList> retrieveAllServiceExecutionsForActor(Integer personId, ZonedDateTime startDate,
                                                                        ZonedDateTime endDate) {
    ...
    @Override
    @Transactional(noRollbackFor = {RuntimeException.class, SQLGrammarException.class}, readOnly = true)
    public List<ServiceResultList> retrieveAllServiceExecutions(ZonedDateTime startDate, ZonedDateTime endDate) {
    ...
    @Override
    public List<ActorResponse> getLastActorResponsesForConceptQuestion(Integer patientId, Integer actorId,
                                                                       Integer conceptQuestionId, Integer pageSize) {
        return actorResponseDao.findAllByCriteria(new LastResponseCriteria()
                .setActorId(actorId)
                .setPatientId(patientId)
                .setConceptQuestionId(conceptQuestionId)
                .setLimit(pageSize), null);
    }

    @Override
    public List<ActorResponse> getLastActorResponsesForTextQuestion(Integer patientId, Integer actorId, String textQuestion,
                                                                    Integer pageSize) {
        return actorResponseDao.findAllByCriteria(new LastResponseCriteria()
                .setActorId(actorId)
                .setPatientId(patientId)
    ...
    @Override
    public List<ActorResponse> getLastActorResponsesForServiceType(Integer patientId, Integer actorId, String serviceType,
                                                                   Integer limit) {
        return actorResponseDao.findAllByCriteria(new LastResponseCriteria()
                .setPatientId(patientId)
                .setActorId(actorId)
    ...
    public List<ScheduledService> getScheduledServicesByPatientIdAndActorId(Integer patientId, Integer actorId) {
        return findAllByCriteria(
                ScheduledServiceCriteria.forPatientTemplateActorId(actorId).withPatientTemplatePatientId(patientId));
    }
    ...
    private void throwExceptionIfMissing(Object entity, Integer id, String entityName) throws EntityNotFoundException {
        if (entity == null) {
            throw new EntityNotFoundException(String.format("Could not find %s with identifier: %s", entityName, id));
        }
    }
    ...
    private List<ServiceResultList> retrieveAllServiceExecutions(List<Template> templates, ZonedDateTime startDate,
                                                                 ZonedDateTime endDate) {
        ...
        for (Template template : templates) {
            if (isTemplateSupportsOptimizedQuery(template)) {
                ...
                List<PatientTemplate> patientTemplates =
                        patientTemplateService.findAllByCriteria(PatientTemplateCriteria.forTemplate(template.getId()));
    ...
    private List<ServiceResultList> retrieveAllServiceExecutionsFromTemplate(Template template, ZonedDateTime startDate,
                                                                             ZonedDateTime endDate) {
    ...
    private List<ServiceResultList> retrieveAllServiceExecutions(List<PatientTemplate> patientTemplates,
                    ...
                    if (LOGGER.isTraceEnabled()) {
                        LOGGER.trace(String.format(
                                "PatientTemplate %d is disabled, so applying the max execution date or now as " +
                                        "the end date (%s instead of %s)", patientTemplate.getId(),
    ...
    private ZonedDateTime getMaxExecutionDate(PatientTemplate patientTemplate) {
        ...
        if (services.size() >= ONE && services.get(0).getGroup() != null &&
            ...
            if (LOGGER.isTraceEnabled()) {
                LOGGER.trace(String.format("PatientTemplate %d ('%s') has got already scheduled tasks at the future, " +
                                "so the last execution date will be applied as the end date", patientTemplate.getId(),
    ...
    private void checkIfStatusIsNotPendingOrFuture(ServiceStatus status) {
        if (ServiceStatus.PENDING.equals(status) || ServiceStatus.FUTURE.equals(status)) {
            throw new IllegalArgumentException(String.format("%s status cannot be registered", status.name()));
        }
    }
    ...
    private void registerResponseForScheduledService(Integer scheduledId, Integer questionId, String textQuestion,
        ...
        registerResponse(actorId, patientId, sourceId, sourceType, questionId, textQuestion, responseId, textResponse,
                timestamp);
    }

    private void logException(PatientTemplate patientTemplate, Exception e) {
        try {
            LOGGER.error(String.format("Cannot execute query for patientTemplate with id %d (template with id %d - '%s'), " +
                            "related to patient %s (uuid: %s)" +
                            "The execution of this particular patient template will be skipped.", patientTemplate.getId(),
                    patientTemplate.getTemplate().getId(), patientTemplate.getTemplate().getName(),
                    patientTemplate.getPatient().getPersonName().getFullName(), patientTemplate.getPatient().getUuid()), e);
        } catch (Exception ex) {
