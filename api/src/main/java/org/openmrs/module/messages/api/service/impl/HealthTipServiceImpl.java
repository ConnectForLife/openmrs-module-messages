...
public class HealthTipServiceImpl implements HealthTipService {
    ...
    private static final String HEALTH_TIP_QUESTION_CONCEPT_UUID = "e7541654-c327-4fee-8e48-f729946c7791";
    ...
    public Concept getNextHealthTipToPlay(Integer patientId, Integer actorId, String category) {
        PatientTemplate healthTipPatientTemplate = findPatientTemplateByPatientAndActorAndService(patientId, actorId,
                HEALTH_TIP_SERVICE_NAME);
        Integer nextHealthTipIdToPlay = null;
        if (healthTipPatientTemplate != null) {
            if (StringUtils.isNotBlank(category)) {
                nextHealthTipIdToPlay = getNextHealthTipIdFromParticularCategory(patientId, actorId, category);
            } else {
                List<String> healthTipCategories = Arrays.asList(getTemplateFieldValue(healthTipPatientTemplate,
                        CATEGORIES_OF_THE_MESSAGE_FIELD_NAME).split(","));
                nextHealthTipIdToPlay = getNextHealthTipIdFromAllCategories(patientId, actorId, healthTipCategories);
            }
        }

        return nextHealthTipIdToPlay != null ? Context.getConceptService().getConcept(nextHealthTipIdToPlay) : null;
    }
    ...
    private Integer getNextHealthTipIdFromParticularCategory(Integer patientId, Integer actorId, String category) {
        Integer nextHealthTipId = null;
        Concept categoryConcept = conceptService.getConceptByName(category);
        if (categoryConcept != null) {
            List<ConceptSet> conceptSetsByCategory = conceptService.getConceptSetsByConcept(categoryConcept);
            ...
            List<Integer> healthTipsIdsAlreadyHeard = getHealthTipsIdsAlreadyHeard(patientId, actorId,
                    possibleHealthTipIds);
    ...
    private Integer getNextHealthTipIdFromAllCategories(Integer patientId, Integer actorId,
                                                        List<String> healthTipCategories) {
        Integer nextHealthTipId = null;
        if (CollectionUtils.isNotEmpty(healthTipCategories)) {
            List<ConceptSet> allConceptSetsByCategories = getAllConceptSetsByCategories(healthTipCategories);
            sortConceptSetsBySortWeight(allConceptSetsByCategories);
            List<Integer> possibleHealthTipIds = getAllConceptIdsByConceptSets(allConceptSetsByCategories);
            List<Integer> healthTipsIdsAlreadyHeard = getHealthTipsIdsAlreadyHeard(patientId, actorId,
                    possibleHealthTipIds);
            nextHealthTipId = getNextHealthTipIdToPlay(possibleHealthTipIds, healthTipsIdsAlreadyHeard);
        } else if (LOGGER.isDebugEnabled()) {
    ...
    private Integer getNextHealthTipIdToPlay(List<Integer> possibleHealthTipIds, List<Integer> healthTipsIdsAlreadyHeard) {
    ...
    private List<Integer> getHealthTipsIdsAlreadyHeard(Integer patientId, Integer actorId,
        ...
        List<ActorResponse> healthTipActorResponses = messagingService.getLastActorResponsesForConceptQuestion(
    ...
    private PatientTemplate findPatientTemplateByPatientAndActorAndService(Integer patientId, Integer actorId,
                                                                           String serviceName) {
        PatientTemplateCriteria criteria = PatientTemplateCriteria.forPatientAndActor(patientId, actorId);
