...
public final class DateUtil {
    ...
    /**
     * The main Clock used by all methods of this class. This facilitates unit tests which needs different time zones.
     */
    ...
    /**
     * Gets the default user timezone as configured by {@link ConfigConstants#DEFAULT_USER_TIMEZONE} global property or
    ...
    public static ZoneId getDefaultUserTimeZone() {
        ...
        if (Context.isSessionOpen()) {
            result = ofNullable(Context.getAdministrationService().getGlobalProperty(ConfigConstants.DEFAULT_USER_TIMEZONE))
                    .map(ZoneId::of)
    ...
    /**
     * Obtains a date time using milliseconds from the epoch of 1970-01-01T00:00:00Z and default user timezone.
     *
    ...
    public static ZonedDateTime ofEpochMilliInUserTimeZone(long epochMilli) {
        return ZonedDateTime.ofInstant(Instant.ofEpochMilli(epochMilli), DateUtil.getDefaultUserTimeZone());
    }

    /**
     * Converts ZonedDateTime to Date.
     * <p>
     * The result will represent the same point in time, but the information about
     * {@code date}'s timezone is lost.
     * </p>
     *
    ...
    /**
     * Converts Date object which was read from OpenMRS database into an ZonedDateTime with a default system timezone.
     * <p>
     * The date times in general OpenMRS are saved into database with timezone of the JVM the app is running on. The
     * database will convert the date&time from connection timezone (JVMs) into UTC, for both read and write so the
     * database
    ...
    public static ZonedDateTime convertOpenMRSDatabaseDate(Date dbDate) {
        ...
        if (dbDate instanceof java.sql.Date) {
            ...
            result = ZonedDateTime.of(sqlDateOnly.toLocalDate(), LocalTime.MIDNIGHT, getDefaultSystemTimeZone());
        } else {
    ...
    /**
     * Obtains a string representation of a date time where, the Time Zone of the {@code dateTime} is changed to default user
     * time zone and the {@link MessagesConstants#DEFAULT_SERVER_SIDE_DATETIME_FORMAT} is used to format the date time.
     *
     * @param dateTime the date time to format, not null
     * @return the string representation of {@code dateTime} in DEFAULT_SERVER_SIDE_DATETIME_FORMAT format, never null
     */
    ...
    /**
     * Obtains a string representation of a date where, the Time Zone of {@code date} is changed to default user time
     * zone and the {@link MessagesConstants#DEFAULT_SERVER_SIDE_DATE_FORMAT} is used to format the date time.
     *
     * @param date the date to format, not null
     * @return the string representation of {@code date} in DEFAULT_SERVER_SIDE_DATE_FORMAT format, never null
    ...
    /**
     * Obtains a string representation of a date where, the Time Zone of {@code date} is changed to default user time zone
     * and
     * the {@link MessagesConstants#DEFAULT_FRONT_END_DATE_FORMAT}
     *
     * @param date the date to format, not null
     * @return the string representation of {@code date} in DEFAULT_FRONT_END_DATE_FORMAT format, never null
    ...
    /**
     * Converts string representation of some date in {@link MessagesConstants#DEFAULT_SERVER_SIDE_DATE_FORMAT} to string
     * representation in {@link MessagesConstants#DEFAULT_FRONT_END_DATE_FORMAT}.
     *
    ...
    public static String convertServerSideDateFormatToFrontend(String date) {
        ...
        final LocalDate localdate =
                LocalDate.parse(date, DateTimeFormatter.ofPattern(MessagesConstants.DEFAULT_SERVER_SIDE_DATE_FORMAT));
        return localdate.format(DateTimeFormatter.ofPattern(MessagesConstants.DEFAULT_FRONT_END_DATE_FORMAT));
    }
     ...
     * @return the string representation of {@code dateTime} in {@code toFormat} format or empty string if the
     * {@code dateTime is null}
     */
